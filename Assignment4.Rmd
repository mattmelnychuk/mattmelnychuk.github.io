...
# square root of x, code from assignment
sqrt(x)
sum(sqrt(x))
log(sum(sqrt(x)))

#below uses pipe operators
sqrt(x) %>% sum %>% log

# log base 2 of 16, code from assignment
log(16, 2)
...
```{r}
df %>% 
  #counting Sept, Oct, Nov, Dec as Fall
  filter(month >=9) %>% 
  filter(date <='2020-12-31') %>% 
  group_by(month2) %>% 
...
df %>% 
  filter(weekend==1) %>% 
  group_by(month) %>% 
  summarise(total_weekend_death = sum(death)) %>% 
  arrange(-total_weekend_death)
```
